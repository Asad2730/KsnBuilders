...
export default function Records({ onClick, cid, partner }) {
    ...
    const getPartners = () => {
        ...
        console.log(url + `getRecordForPartner/${uid}`)
        axios.get(url + `getRecordForPartner/${uid}`)
            .then(res => {
                let ttl = 0;
                for (let i = 0; i < res.data.length; i++) {
                    ttl += res.data[i]['amountpaid'];
            ...
            <div className='mt-4'>
                <table className="w-full shadow text-center bg-white">
                    ...
                    <tbody className='text-center'>
                        {record.map((i) => (
                            <tr key={i.id}>
                                <td className="py-5">{i.date[8]}{i.date[9]}</td>
                                <td className="py-5">{i.date[5]}{i.date[6]}</td>
                                <td className="py-5">{i.date[0]}{i.date[1]}{i.date[2]}{i.date[3]}</td>
